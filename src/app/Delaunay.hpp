...
#ifndef AVP_DELAUNAY_H
#define AVP_DELAUNAY_H
#include <algorithm>
#include <iostream>
#include <vector>
...
struct Point
{
  ...
  Point() : x{0}, y{0}, nodeId{0} {}
  Point(T _x, T _y, int _nodeId) : x{_x}, y{_y}, nodeId{_nodeId} {}

  template <typename U>
  Point(U _x, U _y, int _nodeId) : x{static_cast<T>(_x)}, y{static_cast<T>(_y)}, nodeId{_nodeId}
  {
  }
  ...
  for (auto const& pt : points)
  {
    ...
    edges.erase(
        std::remove_if(edges.begin(), edges.end(),
                       [&](Edge<T> const& e) { return remove[&e - &edges[0]]; }),
        edges.end());

    /* Update triangulation. */
    for (auto const& e : edges)
    {
      tmps.push_back({e.p0, e.p1, {pt.x, pt.y, pt.nodeId}});
    }
    d.triangles = tmps;
  }
